# Evidence: the thread from end to end

Every run here is real: a live Chromium against the Flask app in [`target_app/`](../../target_app),
captured on 2026-09-15. The discovery runs are genuine runs driven by an LLM (Gemini,
...
`SESSION_TIMEOUT_SECONDS=1` and the session was left to idle after sign on, so it expired part
way through the run. The result is `recoverable`, not `business_outcome`: the caller learned
nothing about the member, and the artifact carries the recovery hint *"Replay the
operator_login prerequisite, then retry this capability."* That run also shows the same
artifact executing against a **second deployment on a different port**, because artifacts store
relative paths, not hosts.
...
These runs were not interactive, so the intervention was recorded rather than blocking on a
terminal. Interactively (the default), the browser is **visible rather than headless** and the
run pauses with the live window under the operator's control until they press Enter, the same
window the automation was driving, not a copy of it.

## Reproducing this

The committed artifacts ship **cold**, with `known_outcomes` empty, because a discovery run
